...
puts {
#############################################################################################################################
#TEST CASE ID :TC_ERTR_0306                                                                                                  
#Description  :Verify that when Firewall Config is set to Medium, ICMP message from WAN to WAN IP of WG should not be allowed
##############################################################################################################################
}
...
if {[regexp {.*Lost.*=.*\((.*)% loss\)} $outPing match lossPercent] == 1} {
        ...
        puts "Ping successful from WAN to WAN IP of Gateway when Firewall is set to Medium"
        } else {
        set passFlag [expr $passFlag + 1];
        puts "Ping not successful from WAN to WAN IP of Gateway when Firewall is set to Medium."
...
set voutput "";
set voutput [exec java -cp $ClassPath $Class $oui $SNno $deviceType SetParameterValue Device.X_CISCO_COM_Security.Firewall.FirewallLevel Low string];
puts $voutput;
