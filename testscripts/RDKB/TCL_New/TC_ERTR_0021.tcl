...
puts {
######################################################################################################################### 
#TEST CASEID :TC_ERTR_0021 																							    
#Description  :Verify that 802.11n, 802.11a, 802.11ac clients should be able to connect to WG when mode is  "a/n/ac" 
...
puts {
######################################################################################################################### 
#Step 1 :Configuring the WG with Operating Standard "a/n/ac" for 5GHz radio.                                                                         			 
#########################################################################################################################
}

set output1 "";
set output1 [exec java -cp $ClassPath $Class $oui $SNno $deviceType SetParameterValue Device.WiFi.Radio.$ri5.OperatingStandards a,n,ac string];
puts $output1;
...
puts {
################################################################################
#Step 5 :Validating the output and checking for Radio Type "a" or "n" or "ac"
################################################################################
}
...
if {[regexp { .*There.*Radio type.*: (.*) Auth} $outInt match rtype] == 1 } {
	puts "Expected Radio Type : '802.11a' or '802.11n' or '802.11ac' ."
	if {[regexp {802.11a} $rtype match] == 1 || [regexp {802.11n} $rtype match] == 1 || [regexp {802.11ac} $rtype match] == 1} {
