...
puts {
####################################################################################################################################################	
#TESTCASEID : TC_ERTR_0354															
#Description: Verify that SSID name for 5GHZ band obtained through parameter (Device.WiFi.SSID.2.SSID) matches 
#with the SSID name obtained from Wireless Client.             
######################################################################################################################################################
 }
...
puts {
###########################################################################################################
#step 4 :Get the value of the parameter (Device.WiFi.SSID.$si5.SSID)                                                                   
###########################################################################################################
}

set response [device_set_get_param $ClassPath $Class $oui $SNno $deviceType GetParameterValue Device.WiFi.SSID.$si5.SSID null null {rType output}];
...
puts $passFlag;
set hy [regexp {.*SSID.*: (.*)BSSID} $outInt match opauth] ;
#puts $hy;
#puts $match;
#puts $opauth;

if {[regexp {.*SSID.*: (.*)BSSID} $outInt match opauth] == 1 } {
...
puts $g1;
       if {$g1 == 0} {
        puts "SSID NAME matching";
        set passFlag [expr $passFlag + 1];
        } else {
set failFlag [expr $failFlag + 1];
puts "SSID NAME not matching";
