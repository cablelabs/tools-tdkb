...
puts {
######################################################################################################################### 
#TEST CASEID :TC_ERTR_0139 											
#Description  :Verify that MAC Filter based Access control deny specific clients that are connected via 5GHz radio
#########################################################################################################################
 }
...
set output1 "";
set output1 [exec java -cp $ClassPath $Class $oui $SNno $deviceType SetParameterValue Device.WiFi.AccessPoint.$si5.X_CISCO_COM_MACFilter.FilterAsBlackList true boolean];
puts $output1;
...
set output2 "";
send "curl -X PUT -H \"Authorization: Basic d2VicGFfcmVsZWFzZTEuMA==\" -H \"content-type:application/json\" -H \"X-Webpa-Atomic:true\" -k -i https://api.webpa.comcast.net:8090/api/v2/device/mac:$SNnoWithoutColon/config/Device.WiFi.AccessPoint.$si5.X_CISCO_COM_MacFilterTable. -d '{\"0\":{\"DeviceName\": \"Automation4-PC\",\"MACAddress\": \"$m1\"}}'\r";
...
puts {
######################################################################################################################### 
#Step 3 :Telneting to WLAN Client                                
#########################################################################################################################
}
...
set output5 "";
set otuput5 [exec java -cp $ClassPath $Class $oui $SNno $deviceType SetParameterValue Device.WiFi.AccessPoint.$si5.X_CISCO_COM_MACFilter.Enable false boolean];
puts $output5;
