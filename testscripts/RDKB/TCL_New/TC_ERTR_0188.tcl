...
puts {
######################################################################################################################### 
#TESTCASE ID: TC_ERTR_0188
#Description:Verify that on disabling DMZ, FTP access should not be allowed from WAN to WLAN
#########################################################################################################################
} 
...
puts {
############################################################################################
#Step 5 :Verifying the FTP access to WLAN Client
############################################################################################
}
if {[regexp {.*230.*} $outFtp match] == 1} {
        set passFlag [expr $passFlag + 1];
        puts "FTP access successful from WAN to WLAN when firewall is set to Low and DMZ is enabled."
                } else {
        set failFlag [expr $failFlag + 1];
        puts "FTP access not successful. Blocked from WAN to WLAN"
}
...
puts {
############################################################################################
#Step 8 :Verifying the FTP access to WLAN Client
############################################################################################
}
if {[regexp {.*230.*} $outFtp match] == 1} {
        set failFlag [expr $failFlag + 1];
        puts "FTP access successful from WAN to WLAN when firewall is set to Low and DMZ is disabled."
                } else {
        set passFlag [expr $passFlag + 1];
        puts "FTP access not successful. Blocked from WAN to WLAN when DMZ is disabled"
}
