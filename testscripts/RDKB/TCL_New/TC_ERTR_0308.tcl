...
puts {
#########################################################################################################################
#TEST CASE ID :TC_ERTR_0308                                                                                             
#Description  :Verify that ICMP request is blocked from WAN to WAN IP of gateway on setting the blocking rule in 
...
puts {
##########################################################################################################
#Step 1 : Set Firewall mode to Custom
##########################################################################################################
}
...
set voutput2 "";
set voutput2 [exec java -cp $ClassPath $Class $oui $SNno $deviceType SetParameterValue Device.X_CISCO_COM_Security.Firewall.FilterAnonymousInternetRequests true boolean];
puts $voutput2;
...
if {[regexp {.*Lost.*=.*\((.*)% loss\)} $outPing match lossPercent] == 1} {
        ...
        puts "Ping not successful from WAN to WANIP of Gateway when firewall is set to Custom"
        } else {
        set failFlag [expr $failFlag + 1];
        puts "Ping successful from WAN to WANIP of Gateway when firewall is set to Custom"
...
puts {
##########################################################################################################
#Step 5 : Reverting the Firewall settings to its default value                                                                   
##########################################################################################################
}
set soutput "";
set soutput [exec java -cp $ClassPath $Class $oui $SNno $deviceType SetParameterValue Device.X_CISCO_COM_Security.Firewall.FilterAnonymousInternetRequests false boolean];
puts $soutput;
