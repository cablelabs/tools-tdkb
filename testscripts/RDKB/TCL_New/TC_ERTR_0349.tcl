...
puts {
##########################################################################################################################################
#TEST CASEID :TC_ERTR_0349                                                                                                   
#Description  :Verify that when Firewall Config is set to Custom, ICMP message from WLAN to WAN should be allowed
##########################################################################################################################################
}
...
puts {
################################################################################
#Step 3 :Trying to Telnet to WLAN Client and Ping to WAN Client
################################################################################
}
...
if {[regexp {.*Lost.*=.*\((.*)% loss\)} $outPing match lossPercent] == 1} {
        ...
        if {$lossPercent == 0} {
        set passFlag [expr $passFlag + 1];
        puts "Ping successful from WLAN to WAN when firewall is set to custom"
        } else {
        set failFlag [expr $failFlag + 1];
        puts "Ping not successful from WLAN to WAN when firewall is set to custom"
