...
puts {
#############################################################################################################
#TESTCASEID :TC_ERTR_0310                                                                                                    
#Description  :Verify that enabling 2.4GHz radio gets successfully logged in Wi-Fi Log File of RDK logger
#############################################################################################################
 }
...
set result "PASSED";
puts "The Result of fetch_validate_ rdkLogs is $response";
if {![regexp {pass} $response]} {
puts "Fail to log results in WIFI lodfile of rdklogger";
set result "FAILED";
#exit 0;
}
set passContent "Test Result : $result$~";
