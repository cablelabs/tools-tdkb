...
puts {
######################################################################################################################################
#TEST CASEID :TC_ERTR_0178
#Description  :Verify that Wi-Fi client retains the same IP address when disconnected and reconnected when the Lease period expires
######################################################################################################################################
}
...
set output5 "";
set output5 [exec java -cp $ClassPath $Class $oui $SNno $deviceType SetParameterValue Device.DHCPv4.Server.Pool.1.LeaseTime 120 int];
puts $output5;
...
if {$ip == $ip1} {
puts "Wi-Fi Client retains the same IP address when disconnected and reconnected when the Lease period expired";
set passFlag [expr $passFlag + 1];
} else {
puts "Wi-Fi Client did not retain the same IP address when disconnected and reconnected when the Lease period expired"
set failFlag [expr $failFlag + 1];
}
