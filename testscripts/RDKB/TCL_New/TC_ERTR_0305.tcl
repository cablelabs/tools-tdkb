...
puts {
#########################################################################################################################
#TEST CASE ID :TC_ERTR_0305                                                                                              
#Description  :Verify that when Firewall Config is set to Low, ICMP message from WAN to WAN IP of WG should be allowed
#########################################################################################################################
}
...
puts {
##########################################################################################################
#Step 1 :Set Firewall mode to Low
##########################################################################################################
}
set output1 "";
...
if {[regexp {.*Lost.*=.*\((.*)% loss\)} $outPing match lossPercent] == 1} {
        ...
        puts "Ping successful from WAN to WAN IP of gateway when firewall is set to Low"
        } else {
        set failFlag [expr $failFlag + 1];
        puts "Ping not successful from WAN to WAN IP of gateway when Firewall is set to Low."
