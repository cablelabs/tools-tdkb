...
puts {
#########################################################################################################################
#TEST CASE ID :TC_ERTR_0075                                                                                                  
#Description  :Verify that when Firewall Config is set to Low, FTP access from WLAN to WAN should be allowed.
#########################################################################################################################
}
...
if {[regexp {.*230.*} $outFtp match] == 1} {
        set passFlag [expr $passFlag + 1];
        puts "FTP access is allowed from WLAN to WAN when firewall is set to Low"
                } else {
        set failFlag [expr $failFlag + 1];
