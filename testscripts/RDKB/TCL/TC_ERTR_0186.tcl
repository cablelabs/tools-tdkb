...
puts {
######################################################################################################################### 
#TESTCASE ID: TC_ERTR_0186
#Description:Verify that on disabling DMZ, HTTP traffic should not passthrough from WAN to WLAN
#########################################################################################################################
} 
...
if {[regexp {.*200 OK.*} $outHttp match] == 1} {
        ...
        puts "HTTP Request successful from WAN to WLAN when firewall is set to Low and DMZ is enabled." 
		} else {
...
puts {
############################################################################################
#Step 8 :Verifying the reachability of the HTTP message
############################################################################################
}
if {[regexp {.*200 OK.*} $outHttp match] == 1} {
        set failFlag [expr $failFlag + 1];
        puts "HTTP Request successful from WAN to WLAN when firewall even after DMZ is disabled"
                } else {
        set passFlag [expr $passFlag + 1];
        puts "HTTP Request not successful.Blocked from WAN to WLAN after disabling DMZ"
}
