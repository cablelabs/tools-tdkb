...
puts {
######################################################################################################################### 
#TEST CASE ID :TC_ERTR_0082								
#Description : Verify Port forwarding with HTTP service running on Wi-Fi Client and Firewall mode set to Medium, 
#HTTP traffic should be successful from WAN to WLAN
#########################################################################################################################
} 
...
puts {
##########################################################################################################
#Step 2:Set Firewall to Medium and configure Port Forwarding rule
##########################################################################################################
}
proc commented {} {
set voutput1 "";
...
set output2 "";
set output2 [exec java -cp $ClassPath $Class $oui $SNno $deviceType SetParameterValue Device.X_CISCO_COM_Security.Firewall.FirewallLevel Medium string];
puts $output2;
...
if {[regexp {.*200 OK.*} $outHttp match] == 1} {
        ...
        puts "HTTP Request successful from WAN to WLAN when firewall is set to Medium and Port Forwarding is enabled." 
		} else {
