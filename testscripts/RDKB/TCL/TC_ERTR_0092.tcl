...
puts {
######################################################################################################################### 
#TESTCASE ID: TC_ERTR_0092
#Description  :Verify DMZ with HTTPs service running on Wi-Fi Client and Firewall mode set to low,
...
puts {
##########################################################################################################
#Step 2:Setting the firewall in Low mode and enable DMZ
##########################################################################################################
}
set voutput1 "";
...
if {[regexp {.*200 OK.*} $outHttp match] == 1} {
        ...
        puts "HTTPs Request successful from WAN to LAN when firewall is set to Low and DMZ is enabled."  
		} else {
        set failFlag [expr $failFlag + 1];
        puts "HTTPs Request not successful.Blocked from WAN to LAN"
} 
