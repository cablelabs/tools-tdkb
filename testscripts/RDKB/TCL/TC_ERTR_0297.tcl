...
puts {
######################################################################################################################### 
#TEST CASE ID :TC_ERTR_0297												
#Description  :Verify that when Firewall Config is set to Medium, Telnet access from LAN to WAN  should be allowed
#########################################################################################################################
} 
...
if {[regexp {.*Trying.*....*} $outTelnet match] == 1 } {
        ...
        if {[regexp {.*ogin:} $outTelnet match] == 1 } {	
	set failFlag [expr $failFlag + 1];
        puts "Telnet Request successful from LAN to WAN when firewall is set to Medium"
	}
	}
 }
}
