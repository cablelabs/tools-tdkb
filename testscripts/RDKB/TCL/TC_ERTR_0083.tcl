...
puts {
######################################################################################################################### 
#TEST CASE ID:TC_ERTR_0083
#Description : Verify Port forwarding with HTTPs service running on Wi-Fi Client and Firewall mode set to Medium, HTTPs traffic should be successful from WAN to WLAN
...
if {[regexp {.*200 OK.*} $outHttp match] == 1} {
        ...
        puts "HTTPs Request successful from WAN to WLAN when firewall is set to Medium and Port Forwarding is enabled." 
		} else {
