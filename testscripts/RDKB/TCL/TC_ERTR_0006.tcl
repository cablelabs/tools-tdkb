...
puts {
######################################################################################################################### 
#TEST CASEID :TC_ERTR_0006 								
Description  :Verify that 802.11b, 802.11g and 802.11n client should  able to connect to CM when modes in CM is "b/g/n"  
...
puts {
######################################################################################################################### 
#Step 1 :Configuring the Cable Modem with the specified Operating Standard for 2.4GHz radio.                                                                         			 
#########################################################################################################################
}
set output1 "";
...
set output2 "";
set output2 [exec java -cp $ClassPath $Class $oui $SNno $deviceType GetParameterValue Device.WiFi.Radio.$ri2.OperatingStandards null null];
puts $output2;
...
puts {
################################################################################
#Step 5 :Validating the output and checking for Radio Type "b" or "g" or "n"
################################################################################
}
...
if {[regexp { .*There.*Radio type.*: (.*) Auth} $outInt match rtype] == 1 } {
	puts "Expected Radio Type : '802.11b' or '802.11g' or '802.11n' "
	if {[regexp {802.11b} $rtype match] == 1 || [regexp {802.11g} $rtype match] == 1 || [regexp {802.11n} $rtype match] == 1} {
