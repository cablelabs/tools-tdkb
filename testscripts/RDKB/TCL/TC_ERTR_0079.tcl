...
puts {
######################################################################################################################### 
#TESTCASE ID: TC_ERTR_0079
#Description: Verify Port forwarding with HTTP service running on Wi-Fi Client and Firewall mode set to High,
...
puts {
################################################################################
#Step 4:Telneting to a WAN Client and sending HTTP request through WAN IP
################################################################################
} 
spawn telnet $wanIP;
set timeout 100;
...
if {[regexp {.*200 OK.*} $outHttp match] == 1} {
        ...
        puts "HTTP Request successful from WAN to WLAN when firewall is set to High and Port Forwarding is enabled." 
		} else {
