...
puts {
#####################################################################################################################################################
#TEST CASEID :TC_ERTR_0172						
#Verify that on configuring DHCP Server Subnet mask to 255.255.255.0, it gets reflected successfully on the Wi-Fi Client connected via 2.4GHz radio
#####################################################################################################################################################
}
...
puts {
################################################################################################
#Step 1 :Set DHCP Server Subnet mask to 255.255.255.0
################################################################################################
}
...
if {[regexp {There is no profile "$ssid2" assigned to the specified interface.} $outpCon match] == 1} {
	...
	set failFlag [expr $failFlag + 1];
	} elseif {[regexp {255\.255\.255\.0} $submask] == 1} {
	puts "Connection Successful";
	puts "Expected Subnet mask is : 255.255.255.0";
