...
puts {
######################################################################################################################### 
#TEST CASEID :TC_ERTR_0019 																							    
#Description  :Verify that only 802.11n clients should be able to connect to WG, when mode is 'n' for frequency band 5GHZ
#########################################################################################################################
 }
...
#if {[regexp { .*$ssid5.*\n.*\n.*\n.*\n.*\n.*\n.*Radio type.*: (.*).*Channel.*: 36} $outInt match rtype] == 1 } {
