...
puts {
################################################################################
#Step 1 :Configuring the Accesspoint with WPA-WPA2-PSK with AES encryption                                                                          			 
################################################################################
}
...
puts {
################################################################################
#Step 5 :Validating the output and checking for WPA/WPA2-PSK security mode		 
################################################################################
}
...
if {[regexp {.*There.*Authentication.*: (.*) Cip} $outInt match opauth] == 1 } {
	...
	if {[regexp {WPA2-Personal} $opauth match] == 1 || [regexp {WPA2-Personal} $opauth match] == 1} {
...
set output6 "";
set output6 [exec java -cp $ClassPath $Class $oui $SNno $deviceType SetParameterValue Device.WiFi.AccessPoint.$si5.Security.X_CISCO_COM_KeyPassphrase wifitest123 string];
puts $output6;
