...
puts {
######################################################################################################################### 
#TEST CASEID :TC_ERTR_0050 																							    
#Description  :Verify that when DHCPv4 server is disabled, WIFI client should not be able to get an IP address
...
puts {
################################################################################
#Step 1 :Disabling the DHCPv4 server.                                                                          			 
################################################################################
}

set output1 "";
set output1 [exec java -cp $ClassPath $Class $oui $SNno $deviceType SetParameterValue Device.DHCPv4.Server.Enable false boolean];
puts $output1;
...
set output2 "";
set output2 [exec java -cp $ClassPath $Class $oui $SNno $deviceType SetParameterValue Device.DHCPv4.Server.Pool.1.Enable false boolean];
puts $output2;
...
set output2 "";
set output2 [exec java -cp $ClassPath $Class $oui $SNno $deviceType SetParameterValue Device.DHCPv4.Server.Enable true boolean];
puts $output2;
if {[regexp {.*Time limit has crossed 2 minutes.*} $output2] == 1 } {
...
set output7  "";
set output7 [exec java -cp $ClassPath $Class $oui $SNno $deviceType SetParameterValue Device.DHCPv4.Server.Pool.1.Enable true boolean];
puts $output7;
