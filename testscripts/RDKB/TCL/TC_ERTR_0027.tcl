...
puts {
################################################################################
#TEST CASE ID :TC_ERTR_0027													   									    
#Description  :Verify whether Wireless client can connect to WG when Guard interval 
is 800ns and Channel BW is 20MHZ for frequency band 5GHZ 			   
################################################################################
 }
...
puts {
################################################################################
#Step 1 :Configuring the 5Ghz radio with Guard interval as 800ns and
Channel Bw 20MHZ                                                                     			 
################################################################################
}

set output1 "";
set output1 [exec java -cp $ClassPath $Class $oui $SNno $deviceType SetParameterValue Device.WiFi.Radio.$ri5.GuardInterval 800nsec string];
puts $output1;
...
if {[regexp {There is no profile "$ssid5" assigned to the specified interface.} $outpCon match] == 1} {
	...
	if {[regexp {10\..*\..*\..*} $ip] == 1} {	
	puts "Connection successful after setting Guard interval \"800ns\" and Channel BW \"20MHZ\""
