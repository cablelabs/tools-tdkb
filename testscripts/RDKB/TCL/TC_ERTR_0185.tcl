...
puts {
#########################################################################################################################
#TEST CASE ID :TC_ERTR_0185
#Description : Verify that on disabling Port Forwarding,FTP should not be allowed from WAN to WLAN
#########################################################################################################################
} 
...
if {[regexp {.*230.*} $outFtp match] == 1} {
        ...
        puts "FTP access successful from WAN to WLAN when firewall is set to Low and Port Forwarding is enabled."
                } else {
...
if {[regexp {.*230.*} $outFtp match] == 1} {
        set failFlag [expr $failFlag + 1];
      puts "FTP access successful from WAN to WLAN when firewall is set to Low and Port Forwarding is disabled."
                } else {
        set passFlag [expr $passFlag + 1];
        puts "FTP access not successful. Blocked from WAN to WLAN"
}
