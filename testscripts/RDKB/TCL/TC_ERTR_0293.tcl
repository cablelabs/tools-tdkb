...
puts {
######################################################################################################################### 
#TEST CASE ID :TC_ERTR_0293																
#Description  :Verify that when Firewall Config is set to Low, FTP access from LAN to WAN  should  be allowed
#########################################################################################################################
} 
...
if {[regexp {.*230.*} $outFtp match] == 1} {
        ...
        puts "FTP access successful from LAN to WAN when firewall is set to Low"
                } else {
