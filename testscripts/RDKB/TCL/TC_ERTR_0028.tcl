...
puts {
################################################################################
#TEST CASE ID :TC_ERTR_0028													   									    
#Description  :Verify whether Wireless client can connect to WG when Guard interval 
is 800ns and Channel BW is 40MHZ for frequency band 5GHZ 			   
################################################################################
 }
...
set output2 "";
set output2 [exec java -cp $ClassPath $Class $oui $SNno $deviceType SetParameterValue Device.WiFi.Radio.$ri5.OperatingChannelBandwidth 40MHz string];
puts $output2;
