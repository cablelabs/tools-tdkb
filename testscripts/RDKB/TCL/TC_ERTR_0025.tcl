...
puts {
################################################################################
#TEST CASE ID :TC_ERTR_0025													   									    
#Description  :Verify whether Wireless client can connect to WG when Guard interval 
is 400ns and Channel BW is 20MHZ for frequency band 5GHZ 		   
################################################################################
 }
...
puts {
################################################################################
#Step 1 :Configuring the 5Ghz radio with Guard interval as 400ns and Channel Bw 20MHZ                                                                     			 
################################################################################
}

set output1 "";
set output1 [exec java -cp $ClassPath $Class $oui $SNno $deviceType SetParameterValue Device.WiFi.Radio.$ri5.GuardInterval 400nsec string];
puts $output1;
...
set output2 "";
set output2 [exec java -cp $ClassPath $Class $oui $SNno $deviceType SetParameterValue Device.WiFi.Radio.$ri5.OperatingChannelBandwidth 20MHz string];
puts $output2;
...
if {[regexp {There is no profile "RDKB-5" assigned to the specified interface.} $outpCon match] == 1} {
	...
	if {[regexp {10\..*\..*\..*} $ip] == 1} {	
	puts "Connection successful after setting Guard interval \"400ns\" and Channel BW \"20MHZ\""
...
set output6 "";
set output6 [exec java -cp $ClassPath $Class $oui $SNno $deviceType SetParameterValue Device.WiFi.Radio.$ri5.OperatingChannelBandwidth 40MHz string];
puts $output6;
