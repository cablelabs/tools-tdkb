...
puts {
#########################################################################################################################
#TEST CASE ID :TC_ERTR_0183
#Description : Verify that on disabling Port Forwarding, HTTP traffic should not passthrough from WAN to WLAN
#########################################################################################################################
} 
...
puts {
##########################################################################################################
#Step 2 :Setting the firewall to Low and setting port forwarding rule.                                           
##########################################################################################################
}
proc commented {} {
set voutput1 "";
...
set output3 "";
set output3 [exec java -cp $ClassPath $Class $oui $SNno $deviceType SetMultipleParameterValues  Device.NAT.PortMapping.$P.Enable,Device.NAT.PortMapping.$P.ExternalPort,Device.NAT.PortMapping.$P.ExternalPortEndRange,Device.NAT.PortMapping.$P.Protocol,Device.NAT.PortMapping.$P.InternalClient,Device.NAT.PortMapping.$P.Description true,80,80,BOTH,$Ipval,HTTP boolean,unsignedInt,unsignedInt,string,string,string];
puts $output3;
...
if {[regexp {.*200 OK.*} $outHttp match] == 1} {
        ...
        puts "HTTP Request successful from WAN to WLAN when firewall is set to Low and port forwarding is enabled." 
		} else {
        set failFlag [expr $failFlag + 1];
        puts "HTTP Request not successful.Blocked from WAN to WLAN"
} 
...
puts {
############################################################################################
#Step 8 :Verifying the reachability of the HTTP request
############################################################################################
}
if {[regexp {.*200 OK.*} $outHttp match] == 1} {
        set failFlag [expr $failFlag + 1];
        puts "HTTP Request successful from WAN to WLAN when firewall is low and Port Forwarding is disabled"
                } else {
        set passFlag [expr $passFlag + 1];
        puts "HTTP Request not successful.Blocked from WAN to WLAN after disabling Port Forwarding"
}
