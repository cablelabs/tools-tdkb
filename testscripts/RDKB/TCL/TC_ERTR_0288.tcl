...
puts {
######################################################################################################################### 
#TEST CASE ID :TC_ERTR_0288														
#Description  :Verify that when Firewall Config is set to High, FTP access from LAN to WAN  should be blocked
#########################################################################################################################
} 
...
if {[regexp {.*230.*} $outFtp match] == 1} {
        ...
        puts "FTP access successful from LAN to WAN when firewall is set to High"
                } else {
       
        puts "FTP access not successful.Blocked from LAN to WAN when Firewall is set to High"
