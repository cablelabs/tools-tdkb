...
puts {
######################################################################################################################### 
#TESTCASE ID: TC_ERTR_0088
#Description  :Verify DMZ with HTTP service running on Wi-Fi Client and Firewall mode set to Medium, HTTP traffic should be successful from WAN to WLAN
#########################################################################################################################
} 
...
if {[regexp {.*200 OK.*} $outHttp match] == 1} {
        ...
        puts "HTTP Request successful from WAN to WLAN when firewall is set to Medium and DMZ is enabled." 
		} else {
