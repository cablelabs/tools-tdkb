...
puts {
######################################################################################################################### 
#TEST CASE ID :TC_ERTR_0038																						    
#Description  :Verify whether automatic channel selection can be enabled for 5GHZ radio band and 
...
puts {
######################################################################################################################### 
#Step 1 :Enabling the autochannel enable for 5GHz Radio.                                                                           			  
#########################################################################################################################
}

set output1 "";
set output1 [exec java -cp $ClassPath $Class $oui $SNno $deviceType SetParameterValue Device.WiFi.Radio.$ri5.AutoChannelEnable 1 boolean];
puts $output1;
...
set output2 "";
set output2 [exec java -cp $ClassPath $Class $oui $SNno $deviceType GetParameterValue Device.WiFi.Radio.$ri5.AutoChannelEnable null null];
puts $output2;
...
if {[regexp { .*There.*Channel.*: (.*) Rec} $outInt match channel] == 1 } {
	...
	if { $channel > 11 } {
	puts "For 5GHZ radio";
	puts "Expected Channel number is between 36 and 165";
	puts "Obtained Channel number is $channel";
	set passFlag [expr $passFlag + 1];
		}
	}
...
set output3 "";
set output3 [exec java -cp $ClassPath $Class $oui $SNno $deviceType SetParameterValue Device.WiFi.Radio.$ri5.AutoChannelEnable 1 boolean];
puts $output3;
