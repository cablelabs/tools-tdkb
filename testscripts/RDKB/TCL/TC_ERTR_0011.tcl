...
puts {
################################################################################
#TEST CASE ID :TC_ERTR_0011												
Description  :Verify whether Wireless client can connect to WG when Guard interval 
is 400ns and Channel BW is 20MHZ for frequency band 2.4GHZ
################################################################################
 }
...
puts {
######################################################################################
#Step 1 :Configuring the 2.4Ghz radio with Guard interval as 400ns and Channel Bw 20MHZ                                                                     			 
######################################################################################
}


set output1 "";
set output1 [exec java -cp $ClassPath $Class $oui $SNno $deviceType SetParameterValue Device.WiFi.Radio.$ri2.GuardInterval 400nsec string];
puts $output1;
...
set output5 "";
after 30000 {
catch { exec java -cp $ClassPath $Class $oui $SNno $deviceType SetParameterValue Device.WiFi.Radio.$ri2.GuardInterval Auto string } output5;
}
vwait output5;
puts $output5;
