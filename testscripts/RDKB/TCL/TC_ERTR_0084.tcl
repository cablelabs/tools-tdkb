...
puts {
######################################################################################################################### 
#TEST CASE ID :TC_ERTR_0084										
#Description : Verify Port Forwarding with FTP service running on Wi-Fi Client and Firewall mode set to Medium, 
...
puts {
############################################################################################
#Step 5: Verifying the reachability of the FTP request
############################################################################################
}
if {[regexp {.*230.*} $outFtp match] == 1} {
        set passFlag [expr $passFlag + 1];
        puts "FTP Request successful from WAN to WLAN when firewall is set to Medium and Port Forwarding is enabled."
                } else {
