...
<xml>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check the Beacon type value set in device for 2.4GHz radio using wifi_getApBeaconType HAL API and validate the same</synopsis>
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>To check the Beacon type value set in device for 2.4GHz radio using wifi_getApBeaconType HAL API and validate the same</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XB3. XB6, Emulator, Rpi</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>wifi_getApBeaconType()</api_or_interface_used>
...
2.Python Script will be generated/overrided automatically by Test Manager with provided arguments in configure page (TS_WIFIHAL_2.4GHzGetApBeaconType.py)
3.Execute the generated Script(TS_WIFIHAL_2.4GHzGetApBeaconType.py) using execution page of  Test Manager GUI
...
5.WIFIHAL_GetOrSetParamStringValue function will call Ccsp Base Function named "ssp_WIFIHALGetOrSetParamStringValue", that inturn will call WIFIHAL Library Function wifi_getApBeaconType() function
...
1:wifi_getApBeaconType log from DUT should be available in Agent Console LogCheckPoint
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
	    Beaconvalue = details.split(":")[1].strip()
            if Beaconvalue in beaconTypes:
                print "ApBeaconType is in valid Type %s"%details
   	        tdkTestObj.setResultStatus("SUCCESS");
	        print "TEST STEP 1: Validate the Ap BeaconType";
	        print "EXPECTED RESULT 1: Ap BeaconType should be in ['None', 'Basic', 'WPA', '11i', 'WPAand11i']";
	        print "ACTUAL RESULT 1: AP BeaconType Received: %s"%Beaconvalue;
	        print "[TEST EXECUTION RESULT] : SUCCESS";
	    else:
		print "ApBeaconType is NOT in valid Type %s"%details
