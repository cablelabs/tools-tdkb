...
<?xml version="1.0" encoding="UTF-8"?><xml>
  ...
  <status>FREE</status>
  <synopsis>To get the downstream Data rate and validate it. It should be within the range of 	6 Mbit/s to 35 Mbit/s</synopsis>
  <groups_id/>
...
5.Test Manager will publish the result in GUI as PASS/FAILURE based on the response from TAD stub.</automation_approch>
    <except_output>The downstream data rate should be within 6Mbps to 35 Mbps</except_output>
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult and Min_dataRate <= dataRate <= Max_dataRate:
        ...
        print "TEST STEP 1: Get the Downstream data rate";
        print "EXPECTED RESULT 1: Should get the Downstream datarate successfully";
        print "ACTUAL RESULT 1: Downstream data rate is ",dataRate,"Mbps";
        #Get the result of execution
        print "[TEST EXECUTION RESULT] : SUCCESS";
    else:
        tdkTestObj.setResultStatus("FAILURE");
        print "TEST STEP 1: Get the Downstream data rate";
        print "EXPECTED RESULT 1: Should get the Downstream data rate successfully";
